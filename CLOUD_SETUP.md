...
| 集合名 | 权限 | 用途 |
|--------|------|------|
| **qr_contents** | 所有用户不可读写 | 二维码内容 |
| **qr_submissions** | 所有用户不可读写 | 模板表单提交记录 |
...
| **getContent** | 读取内容并返回 HTML 网页（含模板表单提交） |
| **getSubmissions** | 获取模板表单提交汇总 |
...
3. 左侧 **HTTP 访问服务**
4. 若无默认域名，先 **开通默认域名**
5. **新建** → 关联资源选 **云函数 getContent** → 路径 `/getContent`
6. 复制 HTTP 地址，例如：
   ```
...
3. 页面顶部找到 **全局开关** → **开启**
4. 开启后再 **新建路由**：关联 `getContent`，路径 `/getContent`
5. 若开启时报 `FreePackageDenied`，需先升级云开发为**付费套餐**

> **说明**：当前云开发 HTTP 网关**没有单独的 GET/POST 开关**。路由创建成功后，GET 和 POST 请求都会转发到云函数，由云函数根据 `event.httpMethod` 处理。
...
**Q: 模板二维码扫码后不能填写/无法汇总**  
A: 请确认：  
1. 已重新部署 **getContent**（支持表单提交）和 **getSubmissions**  
2. 已创建集合 **qr_submissions**  
3. HTTP 路由 `/getContent` 已关联云函数 `getContent`（无需单独开 POST 开关）  
4. 旧模板码若仍显示 JSON，需重新部署 getContent 后刷新页面

**Q: 表单提交失败**  
A: 最常见原因是未创建 **qr_submissions** 集合。其次确认 getContent 已部署最新版本（使用原生表单 POST 提交）。

**Q: HTTP 访问服务里找不到 GET/POST 方法开关？**  
A: 正常现象。云开发 HTTP 网关只需配置「域名 + 路径 + 关联云函数」，创建后 GET/POST 均可用，无需额外勾选。
